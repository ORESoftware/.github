...
fn ignored_status(code: Option<i32>) -> Result<bool, Error> {
    match code {
        ...
        other => return Err(Error::GitStatus(other)),
    }
}
...
fn git(fixture: &Path) -> Command {
    ...
    // Isolate fixture commands from inherited repository/configuration/credential overrides.
...
fn behavior(policy: &str) -> Result<(), Error> {
    ...
    // Fixtures remain under ignored tmp/ for diagnosis; never delete caller state.
    return Ok(());
}
...
fn main() {
    // No flags/configuration parser: this is a fixed, read-only repository check.
